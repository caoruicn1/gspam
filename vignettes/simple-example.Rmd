...
```{r}
y <- example_data[c(1:990),104]
df <- example_data[c(1:990),c(-102,-103,-104)]
new_y <- example_data[c(991:1000),104]
new_data <- example_data[c(991:1000),c(-102,-103,-104)]
```
...
prediction_path <- predict(results3,new_data,50)
prediction_path
```

Now predicting using the best model from cross-validation:
```{r}
prediction_cv <- predict(results2,new_data)
prediction_cv
```
